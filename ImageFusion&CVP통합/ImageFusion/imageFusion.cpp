...
void Fusion::sml(IplImage** dst)
{
	...
	long long int arrSml1[7000][4];
	long long int arrSml2[7000][4];
	...
	for (int i = 1; i < size.height - 1; i++)
	{
		...
		for (int j = 1; j < size.width - 1; j++)
		{
			if (ptrMarkers[j]>0) {
				...
				arrSml1[ptrMarkers[j]][0] += ptrsml1[j];
				arrSml1[ptrMarkers[j]][1]++;
				arrSml2[ptrMarkers[j]][0] += ptrsml2[j];
				arrSml2[ptrMarkers[j]][1]++;
			}
	...
	for (int i = 1; i < 7000; i++)
	{
		...
		{
			arrSml1[i][0] /= arrSml1[i][1];
			arrSml1[i][0] *= 2;
			arrSml2[i][0] /= arrSml2[i][1];
			arrSml2[i][0] *= 2;
		}
	}
	...
	for (int i = 1; i < size.height - 1; i++)
	{
		...
		{
			if (ptrMarkers[j]>0) {
				if (ptrsml1[j] >= arrSml1[ptrMarkers[j]][0])
				{
					arrSml1[ptrMarkers[j]][2] += ptrsml1[j];
					arrSml1[ptrMarkers[j]][3]++;
				}
				if (ptrsml2[j] >= arrSml2[ptrMarkers[j]][0])
				{
					arrSml2[ptrMarkers[j]][2] += ptrsml2[j];
					arrSml2[ptrMarkers[j]][3]++;
				}
			}
	...
	for (int i = 1; i < 7000; i++)
	{
		...
		{
			arrSml1[i][2] /= arrSml1[i][3];
			arrSml2[i][2] /= arrSml2[i][3];
		}
	}
	...
	for (int i = 1; i < size.height - 1; i++) 
	{
		...
		for (int j = 1; j < size.width - 1; j++) 
		{
			...
			else if(ptrMarkers[j]>0){
				//if (arrSml1[ptrMarkers[j]] < arrSml2[ptrMarkers[j]]) {
				if (arrSml1[ptrMarkers[j]][2] < arrSml2[ptrMarkers[j]][2]) {
	...
	//		if (ptrsml1[j] < ptrsml2[j])
	//		{
	//			ptrFimg[3 * j] = ptrimg2[j * 3];
	//			ptrFimg[3 * j + 1] = ptrimg2[j * 3 + 1];
	//			ptrFimg[3 * j + 2] = ptrimg2[j * 3 + 2];
	//		}
	//	}
	//}
...
void Fusion::backupsml(IplImage** dst)
{
	if (img1 == NULL) {
		cout << "No Setted Image!!" << endl;
		return;
