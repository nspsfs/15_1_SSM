...
int main()
{
	...
	while (1)
	{
		cout << "1:start record, 2:stop record, 0:EXIT" << endl;
		cin >> input;
		switch (input)
		{
		case 1:
...
void cameraSetup(EdsCameraRef* camera, EdsCameraRef* camera2)
{
	...
	if (err1 == EDS_ERR_OK)
	{
		...
		//Download Parts
		//err1 = EdsSetObjectEventHandler(camera11, kEdsObjectEvent_All, handleObjectEvent, NULL);
		//err2 = EdsSetObjectEventHandler(camera22, kEdsObjectEvent_All, handleObjectEvent, NULL);
	}
...
EdsError downloadImage(EdsDirectoryItemRef directoryItem, int num_camera)
{
	...
	if (err == EDS_ERR_OK)
	{
		...
		if (num_camera == 1)
		{
			//str_path = "[LEFT]" + (string)dirItemInfo.szFileName;
